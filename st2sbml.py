...
PRODUCT_PREFIX = {
    ...
    "binding"  :   "bound ",
    "catabolism"   :   "catabolized ",
    "catalysis"    :   "catalyzed ",
    "conversion"   :   "converted ",
    "deacetylation"    :   "deacetylated ",
    "degradation"  :   "degradated ",
    "demethylation"    :   "demethylated ",
    "dephosphorylation"    :   "dephosporylated ",
    "deubiquitination" :   "deubiquinated ",
    "dissociation" :   "dissociated ",
    "gene_expression" : "expressed ",
    "inactivation" :   "deactivated ",
    "localization" :   None,
    "methylation"  :   "methylated ",
    "negative_regulation"  :  "negatively regulated ",
    "phosphorylation"  :   "phosphorylated ",
    "positive_regulation"  :   "positively regulated ",
    "protein_catabolism"   :   "catabolized ", # degradation
...
def parse_arguments( args = None):
    ...
    parser.add_argument( '--drop-regulations',
                        action = "store_true",
                        dest = "drop_regulations",
                        default = False,
                        help = "whether to drop regulation events (this drops the product and adds the reactant as cause to activated events) - fails when there is a cause on the regulation event")
...
def add_cvterm( term, arguments = DEFAULT_ARGUMENTS):
    ...
    controlled_vocab = libsbml.CVTerm(libsbml.BIOLOGICAL_QUALIFIER);
    check( controlled_vocab.setBiologicalQualifierType(libsbml.BQB_IS), "addSBO term-pt1", arguments = arguments);
    check( controlled_vocab.addResource(term),"addSBO term-pt2", arguments = arguments);
...
def add_reaction( event, model, arguments = DEFAULT_ARGUMENTS):
    ...
    check( reaction.addCVTerm( add_cvterm( "urn:miriam:" + STANDOFF_EVENT_TO_SBO_MAPPING[event.type_lower])), 'set controlled vocab', arguments = arguments);
...
def handle_regulation( event, model, arguments = DEFAULT_ARGUMENTS):
    ...
    if len( event.get_roles( "Theme")) == 0 or any( [isinstance( theme, parse_standoff.EntityTrigger) for theme in event.get_roles( "Theme")]):
        ...
        for theme in event.get_roles( "theme"):
            if isinstance( theme, parse_standoff.EntityTrigger):
                add_reactant( theme.id, reaction, model, arguments = arguments);
            elif isinstance( theme, parse_standoff.Event):
...
def compute_reactants_products_causes( model):
    "Computes dictionaries of species_id : reactions_id"
    reactants = { spec.getId() : [] for spec in model.getListOfSpecies()}
    products = { spec.getId() : [] for spec in model.getListOfSpecies()}
    modifiers = { spec.getId() : [] for spec in model.getListOfSpecies()}
    for reaction in model.getListOfReactions():
        for ref in reaction.getListOfReactants():
            reactants[ref.getSpecies()].append( reaction.getId())
        for ref in reaction.getListOfProducts():
            products[ref.getSpecies()].append( reaction.getId())
        for ref in reaction.getListOfModifiers():
            modifiers[ref.getSpecies()].append( reaction.getId())
...
def cleanup_remove_unconnected_reactions( model, arguments = DEFAULT_ARGUMENTS):
    ...
    for reaction_id in reaction_ids:
        ...
        if len(reaction.getListOfReactants()) == 0 and len(reaction.getListOfProducts()) == 0 and len(reaction.getListOfModifiers()) == 0:
            check( model.removeReaction( reaction_id), 'remove reaction w/o reactants, products and modifiers', arguments = arguments);

def cleanup_drop_regulations( model, arguments = DEFAULT_ARGUMENTS):
    sbo_to_drop = [168,169,170,172]
    reactions_to_drop = [] # reactions without cause and products only linked to another reaction as cause
    species_to_drop = []
    # find all links for product, drop reactions and products and link reactants to previous product links
    reactants, products, modifiers = compute_reactants_products_causes( model)
    for r in model.getListOfReactions():
        if r.getSBOTerm() in sbo_to_drop:
            if len( r.getListOfModifiers()) != 0:
                logging.getLogger( "st2sbml").warning( "Could not drop regulation %s because modifier(s) present." % r.getId())
            elif any( products[ref.getSpecies()] !=  1 for ref in r.getListOfProducts()):
                logging.getLogger( "st2sbml").error( "Product(s) of regulation %s is also a product of other events/reactions. Skipping event!" % r.getId())
            elif len( r.getListOfReactants()) == 0: # no reactants
                reactions_to_drop.append( r.getId())
            else: # reactant there, need to do some rewiring
                reactions_to_drop.append( r.getId())
                reactants_r = [ref.getSpecies() for ref in r.getListOfReactants()]
                # for each product
                for ref in r.getListOfProducts():
                    p = ref.getSpecies()
                    # relink the reactants of r to various reactions linked through p
                    for reactant_reaction in reactants[p]:
                        for reactant in reactants_r:
                            add_reactant( reactant, model.getReaction( reactant_reaction), model)
                    for modifier_reaction in modifiers[p]:
                        for reactant in reactants_r:
                            add_modifier( reactant, model.getReaction( modifier_reaction), model)
                    species_to_drop.append( p)
    for s in species_to_drop:
        check( model.removeSpecies( s), 'remove product species of regulation event', arguments = arguments);
    for r in reactions_to_drop:
        check( model.removeReaction( r), 'remove regulation event', arguments = arguments);
...
def cleanup_remove_unconnected_species( model, arguments = DEFAULT_ARGUMENTS):
    ...
    for species_id in set_of_species.difference( set_of_species_in_reactions):
        check( model.removeSpecies( species_id), 'remove unconnected species', arguments = arguments);
...
def cleanup_remove_empty_compartments( model, arguments = DEFAULT_ARGUMENTS):
    ...
    for compartment_id in compartments_not_used:
        check( model.removeCompartment( compartment_id), 'remove compartment', arguments = arguments)
...
def create_document( trigger, entity_trigger, events, arguments = DEFAULT_ARGUMENTS):
    ...
    ##### CLEANUP
    # remove reactions w/o reactants, products or modifiers
    if arguments.remove_unconnected_reactions:
        cleanup_remove_unconnected_reactions( model, arguments = arguments);
        
    # remove reactions w/o reactants, products or modifiers
    if arguments.drop_regulations:
        cleanup_drop_regulations( model, arguments = arguments);
    
    # remove unconnected species (the ones not involved in any reaction)
    if arguments.remove_unconnected_species:
    ...
    logging.getLogger( "st2sbml").info( "Processing {}: created model with {} species and {} reactions".format( arguments.path, model.getNumSpecies(), model.getNumReactions()))
